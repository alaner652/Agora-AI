...
export function ChatView({ initialMessages, initialSessionId }: ChatViewProps) {
  ...
  const [viewingSessionId, setViewingSessionId] = useState<string | null>(initialSessionId)
            ...
            <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
              {SUGGESTIONS.map(s => (
                <button key={s.text} onClick={() => sendSuggestion(s.text)}
                  className="text-left bg-card/70 backdrop-blur-xl hover:bg-accent border border-border hover:border-primary/40 rounded-xl px-3 py-2.5 transition-colors group">
          ...
          <div className="py-6 px-4 md:px-6 space-y-6 max-w-3xl mx-auto">
            ...
            {askUser && (
              <div className="flex gap-3 justify-start">
                ...
                <div className="bg-card/70 backdrop-blur-xl border border-border rounded-2xl rounded-tl-sm px-4 py-3 max-w-sm shadow-sm">
